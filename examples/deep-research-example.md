# Example: Full Deep Research Report

## Scenario

You are an established seller of **portable Bluetooth speakers** expanding from **Amazon US to Amazon Germany (DE)**.
You need deep intelligence on the top competitor in the German market.

## Request

```
@amazon-competitor-analysis
Deep research mode: ASIN B07P8MQ43H on Amazon DE
My product: portable Bluetooth speaker, IPX7 waterproof + 24h battery + TWS pairing, EUR 39-59
Include all 7 modules, 90-day analysis window
```

## Output: Condensed Full Report

---

# [DE] JBL Go 3 Comprehensive Research Report

> Research Date: 2025.05.20
> Period: 30-day basic + 90-day trend
> Reference Product: Portable BT speaker, IPX7 + 24h battery + TWS, EUR 39-59
> Tools: Seller Sprite, Keepa, SimilarWeb

---

### 1. Core Competitor Basic Info

| Metric | Value |
|--------|-------|
| Brand | JBL (Harman) |
| Core ASIN | B07P8MQ43H |
| Listing Date | 2020.10 |
| Current Price | EUR 34.99 |
| Rating / Reviews | 4.6鈽?(58,342 reviews) |
| Monthly Sales | ~12,500 units |
| BSR | Electronics #42 / Portable BT Speakers #1 |
| Est. Conversion | 12.1% |
| Variants | 8 (colors: black, blue, red, teal, etc.) |

### 2. Traffic Structure Deep Dive

**Level 1:** Natural 72% / Paid 28%

**Level 2:**
- Natural Search: 48%
- Natural Recommendation: 24%
- Paid Search: 18%
- Paid Recommendation: 10%

**Level 3 鈥?Top Traffic Tags:**
- Amazon's Choice: 31%
- Highly Rated: 18%
- Customers Frequently Viewed: 12%
- Trending Now: 8%

**Variant Traffic:** Black (42%), Blue (22%), Red (13%), Others (23%)

### 3. Ad Strategy Full Analysis

| Ad Type | Traffic Share | Campaigns | Core Creative |
|---------|--------------|-----------|--------------|
| SP | 60% | 12 | White bg product shot |
| SB | 25% | 3 | Product lineup + brand |
| SBV | 10% | 1 | 45s lifestyle video |
| SD | 5% | 2 | Retargeting display |
| SDV | 0% | 0 | 鈥?|

**Keyword Distribution:**
- Head: 28% 鈥?"bluetooth speaker", "portable speaker", "JBL speaker"
- Mid-tail: 52% 鈥?"wasserdichter bluetooth lautsprecher", "kleiner bluetooth speaker"
- Long-tail: 20% 鈥?German-specific long queries

**Defense Strategy:** Heavy brand term bidding on all "JBL" variants
**Offense Strategy:** Minimal competitor brand conquest 鈥?brand strength makes it unnecessary

### 4. Promotion Strategy & Price Rhythm

| Type | Discount | Frequency | Duration | Purpose |
|------|---------|-----------|----------|---------|
| Long-term | None | 鈥?| 鈥?| Brand pricing power |
| Short Deal | 15-20% | Every 6-8 weeks | 5-7 days | BSR maintenance |
| Price Cut | EUR 39.99 -> 34.99 | Once (2023) | Permanent | Competitive adjustment |
| Mega-sale | 25% | Prime Day, BFCM | 3-5 days | Peak volume |

**Price History (90 days):** EUR 29.99 (Prime Day) 鈥?EUR 34.99 (regular)

### 5. Creative Assets & Reusable Templates

**Priority:** Main Image Video > Main Image > A+ Content > Secondary Images

**Video Framework:**
1. 0-3s: Outdoor adventure scene (emotional hook)
2. 3-45s: Feature showcase (water splash -> durability -> sound quality -> portability)
3. 45-50s: JBL logo + "JBL Go 3" + Amazon CTA

**Selling Point Order (Images):**
1. IP67 waterproof (splash visual)
2. Compact size comparison
3. 5h battery life infographic
4. Color variants grid
5. Bluetooth 5.1 specs
6. What's in the box

**Reusable Pattern:** Emotional lifestyle opener + feature-claim-evidence sequence + clean brand close

### 6. Off-site Traffic Channel Layout

| Channel | Platform | Key Data | Cadence |
|---------|---------|----------|---------|
| Website | jbl.com | 2.1M monthly visits (DE traffic ~15%) | Event-synced |
| Deal Sites | mydealz.de | 3-4 posts/year, avg 280deg heat | Aligned with Amazon deals |
| YouTube | JBL Global | 480K subs, avg 85K views | 2-3 videos/month |
| Facebook | JBL Deutschland | 210K followers, 1.2% engagement | Daily posts |
| TikTok | @jblaudio | 1.2M followers | 3-5 short videos/week |

**Core Off-site Strategy:** Brand-level content funnel -> Amazon product page. Deal sites used sparingly to protect brand equity.

### 7. SWOT Comparison & Action Plan

| Dimension | JBL Advantage | JBL Weakness | Our Opportunity |
|-----------|-------------|-------------|----------------|
| Product | Brand trust, proven durability | 5h battery vs our 24h | Battery life is our #1 differentiator |
| Price | EUR 34.99 brand premium | Older model, room for undercutting | Position at EUR 39.99 with better specs |
| Traffic | 72% natural (brand search) | New brands can't replicate | Focus on category keywords, not brand |
| Ads | Dominant SP coverage | Low SD/SDV utilization | SD retargeting as under-utilized channel |
| Promos | Infrequent deals | Leaves price gap | Offer launch deal + bundle incentive |
| Creative | Professional, lifestyle-driven | One-size-fits-all global | Localize for German market specifics |
| Off-site | Brand-level ecosystem | No product-specific DE content | Build German YouTube review pipeline |
| Reviews | 58K reviews (moat) | Occasional battery complaints | Seed reviews highlighting 24h battery |

### Monthly Action Checklist

| Task | Details | Budget | Deadline |
|------|---------|--------|----------|
| Product | Add German manual + packaging localization | 鈥?| Week 1 |
| Listing | German keyword optimization (mid-tail focus) | EUR 200 | Week 1 |
| Ads | SP auto campaign -> manual after 2 weeks | EUR 500/mo | Week 1 |
| Promo | 15% launch coupon (30 days) | 鈥?| Week 1 |
| Creative | German-language main image video | EUR 800 | Week 3 |
| Reviews | Vine program enrollment | 鈥?| Week 1 |
| Off-site | Outreach to 5 German tech YouTubers | EUR 1,500 | Week 2-4 |

**Total First Month Budget: ~EUR 3,000**